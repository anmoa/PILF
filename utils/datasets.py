...
def get_dataset(dataset_name: str, img_size: int, patch_size: int, data_root: str = './temp_data') -> Tuple[Any, Any]:
    """
    ...
    Args:
        ...
        patch_size (int): Patch size (used for model configuration, not directly for dataset loading here).
        data_root (str): Root directory where datasets will be downloaded/loaded from.
    Returns:
        Tuple[Any, Any]: A tuple containing the training and test datasets.
    """
    ...
    if dataset_name == 'MNIST':
        ...
        train_dataset = datasets.SVHN(root=data_root, split='train', download=True, transform=transform)
        test_dataset = datasets.SVHN(root=data_root, split='test', download=True, transform=transform)
    else:
